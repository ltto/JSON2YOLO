...
def convert_coco_json(json_dir='../coco/annotations/', out="new_dir", class_anno=None, use_segments=False):
    save_dir = Path(out)  # output directory
    # 检查文件夹是否存在
    if os.path.exists(save_dir):
        shutil.rmtree(save_dir)

    os.makedirs(out, exist_ok=True)
    if class_anno is None:
        class_anno = os.path.join(json_dir, "annotations.json")
    class_map, names = load_coco_anno_class(class_anno)
    data_yaml = {
        "train": "./train/images",
        "val": "./valid/images",
        "test": "./test/images",
    ...
    with open(save_dir / "data.yaml", 'w') as file:
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        ...
        fn_img = Path(save_dir) / json_file.stem.replace('instances_', '') / 'images'  # folder name
        os.makedirs(fn_img)
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            shutil.copy2(Path(json_dir) / "JPEGImages" / f, fn_img / f)
            bboxes = []
            segments = []
            for ann in anns:
                if ann['iscrowd']:
                ...
                cls = class_map[ann['category_id']]  # class
                box = [cls] + box.tolist()
